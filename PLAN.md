...
Social-science research has different artifacts (analysis plans, data dictionaries, methodology decisions, tables, figures), a different unit of work (estimations, robustness tests, validation checks), and different failure modes (wrong-signed coefficients, non-convergent MCMC, omitted confounders, p-hacking risk). Direct use of Matt's skills mostly works but leaves the biggest research-specific leverage on the floor.
...
- ✅ Robustness & sensitivity tests (pre-specified variations)
...
4. **GitHub label vocabulary** beyond the 4 triage states (do we add `methodology`, `plumbing`, `robustness` labels to mirror issue type?).
