...
- **Triage labels** — two flavours: **state labels** (`needs-info`, `ready`, `awaiting-review`, `done`) that move regular issues through the lightweight pipeline, and **kind labels** (`epic`, `autonomous-ok`) that mark categorically-different issues or carry a permanent attribute the state pipeline doesn't capture. `epic` is applied to plan containers created by `to-analysis-plan`; `autonomous-ok` is applied by `to-issues-research` to work that falls within the agent's autonomous scope.
...
> Explainer: The research skills use two flavours of labels. **State labels** move regular work issues through four states: `needs-info` (grilling required), `ready` (methodology locked, spec written, deps closed), `awaiting-review` (work finished — you need to inspect), and `done`. **Kind labels** mark categorically-different issues or carry a permanent attribute: `epic` is the container created by `to-analysis-plan` to anchor a numbered analysis plan to the issue tracker (and doesn't get a state label); `autonomous-ok` is applied by `to-issues-research` to regular work issues that fall within the agent's autonomous scope (and coexists with a state label). To apply any of these the skills need GitHub labels matching strings *you've actually configured*. Pick the strings now; they get written to `docs/agents/triage-labels.md` and (optionally) created on GitHub in step 5.
...
State labels (four-state pipeline): `needs-info` / `ready` / `awaiting-review` / `done`. Kind labels: `epic` (plan containers, not in the state pipeline) and `autonomous-ok` (regular work issues within the agent's autonomous scope; coexists with a state label). See `docs/agents/triage-labels.md`.
...
1. Run `gh label list --json name --jq '[.[].name]'` and diff against the six confirmed label strings (four state + two kind).
...
- Which skills will read which files (`grill-with-docs-research` produces `CONTEXT.md` and ADRs; `to-issues-research` reads them and applies the labels documented in `docs/agents/triage-labels.md`).
